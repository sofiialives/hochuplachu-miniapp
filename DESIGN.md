...
## Visual Language v2 — АКТУАЛЬНЫЙ (редизайн 2026-08, трек G)

> Всё, что ниже этого раздела (начиная с «Overview») — ИСТОРИЧЕСКИЙ референс
> первой версии интерфейса (Anthropic Claude warm canvas: cream + coral +
> slab-serif). Он сознательно сохранён как контекст, но токены приложения с
> редизайна 2026-08 задаёт ЭТОТ раздел. Источник истины по значениям —
> `src/styles/_tokens.scss`.

Интерфейс перекрашен в визуальный язык лендинга **hochuplachu-landing**
(`src/config/site.config.ts` → brand.palette / paletteExtended / tokens):

**Палитра (дефолты; 8 ключей брендируются через brand.json):**
- primary `#FED136` (фирменный жёлтый), on-primary `#3D2000` — на жёлтых
  кнопках ТЁМНЫЙ текст, белый нечитаем;
- ink `#212529`, canvas `#F8F9FA`, surface `#FFFFFF`, surface-card `#F1F3F5`,
  muted `#6C757D`, hairline `#DEE2E6`;
- **все производные — `color-mix()` от `--color-primary`**: hover/active,
  primary-soft (≈ #FFF8E1), primary-softer (≈ #FFF0B8), disabled, градиент CTA
  `--grad-primary` (≈ #FFD828→#F5B800), тени `--shadow-primary(-hover)` и
  текстовый акцент `--color-primary-ink` (45% primary + 55% ink — жёлтый как
  цвет ТЕКСТА на светлом фоне не используется никогда). Благодаря этому
  CatCard (brand.json с primary #d2aa22) получает свои производные сам —
  захардкоженных оттенков бренда в компонентах нет.

**Типографика:** display — **Unbounded 700** (плакатные заголовки, размеры
уменьшены против serif-эпохи: h1 24 / h2 20 / h3 17, секционные h2 в
страницах — 18px); body — **Onest 400/500/700**. Self-hosted woff2 в
`public/fonts` (копии из лендинга), `@font-face` в `global.scss`, preload
display-шрифта в `index.html`. Google-fonts (Cormorant/Inter) удалены.
Mono остаётся системным фолбэком (JetBrains Mono не грузится).

**Формы и глубина:** радиусы укрупнены (md 12 / lg 16 / xl 20, pill),
карточки каталогов — белые поверхности с мягкой тенью `--shadow-card`
вместо голых hairline-рамок; hover — lift (translateY(-2px)) + жёлтая
рамка + `--shadow-card-hover`; брендовые hero/featured-блоки — заливка
primary-soft с radial-подсветкой.

**Движение (только CSS, без @angular/animations):**
- переходы роутера — `withViewTransitions()` + `::view-transition-old/new`
  в global.scss (fade + сдвиг 6–10px, 200/260 мс);
- `.stagger-in` (global.scss) — каскадное появление карточек каталогов
  (40 мс шаг, до 10 детей);
- диалоги/bottom-sheet/тосты — spring-easing `--ease-spring`
  (cubic-bezier(.34,1.45,.64,1), лёгкий overshoot);
- прогресс трафика eSIM — width-transition .6s `--ease-out`, заливка
  `--grad-primary`;
- кнопки/чипы/инпуты — transition background/border-color/box-shadow/
  transform (`--dur-quick` .18s), нажатие чипов — scale(.96);
- `prefers-reduced-motion: reduce` глобально отключает всё движение.

**Bottom-nav:** активный пункт — залитая пилюля `--grad-primary` с
on-primary текстом (жёлтый текст на светлом фоне запрещён).

Админка НЕ редизайнилась отдельно — она наследует базовые токены.
